$(function(){ /* можно обращаться к тегу столько раз, сколько я захочу */
    /* обращаемся к тегу и пoдключаем клик (3)  */
    $('.info-tabs__item').on('click', function (e) {
        /* отменяет обработчика ссылок, чтобы работа быстрее */
        e.preventDefault();
        /* убирает подчеркивание */
        $('.info-tabs__item').removeClass('info-tabs__item--active');
        /* показывает, как раз с помощью того подчеркивания, куда я тыкнулa */
        $(this).addClass('info-tabs__item--active');
        /* убираем класс эктив у того, у кого он написан */
        $('.info-tabs__content').removeClass('info-tabs__content--active');
        /* достаю класс с информальцией из тега, которому присвоила id */
        $($(this).attr('href')).addClass('info-tabs__content--active');

    });
/* показ слайдеров для иныформации с картинкой, текстом */
